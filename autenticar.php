...
if($id_usu != ""){
	...
	if($_SESSION['nivel'] == 'Loja'){	
		$_SESSION['aut_token_045'] = 'xss_010204';			
	}else{
	...
	if($pagina == ""){
		if($_SESSION['nivel'] == 'Loja'){	
		echo '<script>window.location="painel_loja"</script>'; 	
		}else{
		...
		if($_SESSION['nivel'] == 'Loja'){	
		echo '<script>window.location="painel_loja/'.$pagina.'"</script>';  
		}else{
...
if($linhas > 0){
	...
	if($res[0]['ativo'] != 'Sim'){
		$_SESSION['msg'] = 'Em breve seu acesso será liberado!'; 
		echo '<script>window.location="index.php"</script>';  
	}
