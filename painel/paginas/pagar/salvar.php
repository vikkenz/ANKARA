...
//buscar o id do usuario relacionado a loja
$query = $pdo->query("SELECT * FROM usuarios where ref = '$loja'");
...
//validacao
if($descricao == "" and $fornecedor == "0" and $funcionario == "0" and $loja == "0"){
	echo 'Selecione um Fornecedor ou um Funcionário ou uma Descrição ou uma Loja!';
	exit();
}
...
if($fornecedor != "0" and $loja != "0"){
	echo 'Selecione uma loja ou um Fornecedor!';
	exit();
}

if($loja != "0" and $funcionario != "0"){
	echo 'Selecione uma loja ou um Funcionário!';
	exit();
}
...
$query = $pdo->prepare("UPDATE $tabela SET descricao = :descricao, fornecedor = :fornecedor, funcionario = :funcionario, valor = :valor, vencimento = '$vencimento' $pgto, forma_pgto = '$forma_pgto', frequencia = '$frequencia', obs = :obs, arquivo = '$foto', subtotal = :valor, loja = :loja where id = '$id'");
}
